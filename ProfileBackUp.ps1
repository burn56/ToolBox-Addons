...
Updated code by:
	Author: Matt Urbano
	Git: https://github.com/burn56/ToolBox-Addons
#>
...
function Show-ProfileBackup {
	...
	$textbox1 = New-Object 'System.Windows.Forms.ComboBox'
    ...
    $UserCount = 0
    $UserDirList = Get-ChildItem -Path C:\Users -Name
    Foreach ($UserDir in $UserDirList){
                $textbox1.Items.Add($UserDir)
                $UserCount ++
                }
	...
	function listUsers
	{
		$userFolders = Get-ChildItem -path c:\users -Name
		ForEach ($UserDir in $userFolders) { $textbox1.Items.Add($UserDir) }
	}
	
	function backupDirectory
	{
		$Basesource = $TextBox1.Text
        $source = "C:\users\$Basesource"
		$dest = $TextBox2.Text
		...
		$TestSignature = Test-Path "$source\application data\microsoft\signatures"
		if($TestSignature -eq $true){Copy-Item "$source\application data\microsoft\signatures $dest\application data\microsoft\signatures" -Recurse -Force}
